...
# Bubble detector
This project maintains the detector module in [Bubble](https://github.com/Birdiebot/bubble) project, because the module is too large, it does not exist in `bubble_contrib` repository, but is maintained separately. Currently, the repository maintains a armour detector and a rune detector. They are implemented by traditional vision algorithm and convolutional neural network respectively. For details, see the README file under the each packages.
