...
void TRTModule::create_module(const std::string &onnx_file)
{
    ...
    auto onnx_file_path = onnx_file;
    auto cache_file_path = onnx_file_path;
    cache_file_path.replace(cache_file_path.end()-4, cache_file_path.end(), "cache");
    std::ifstream file(cache_file_path);
    if (file.is_open())
    {
        std::cout << "[INFO]: cache file " << cache_file_path<< " exits" << std::endl;
        file.close();
        build_engine_from_cache(cache_file_path.c_str());
    }
    else
    {
        std::cout << "[INFO]: cache file " << cache_file_path<< " not exits, buid cache" << std::endl;
        build_engine_from_onnx(onnx_file_path.c_str());
        cache_engine(cache_file_path.c_str());
    }
...
std::vector<bbox_t> TRTModule::doinfer(const cv::Mat &src) const
{
